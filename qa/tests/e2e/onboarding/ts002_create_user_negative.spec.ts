  ...
  describe('Malformed JSON (NF003)', () => {
    it('should return 400 for malformed JSON payload (NF003)', async function() {
      setTestMetadata({
        ...
        owner: 'QA Team',
        description: '🚨 CRITICAL GAP: No unit test exists for malformed JSON handling. Priority: P1 (High Priority)'
      });
  ...
  describe('Kafka Timeout Handling (KAF003)', () => {
    it('should handle Kafka publish timeout gracefully (KAF003)', async function() {
      setTestMetadata({
        ...
        owner: 'QA Team',
        description: '🚨 CRITICAL GAP - P1 Priority: No unit test exists for Kafka timeout scenario. User creation should succeed even if Kafka times out.'
      });
